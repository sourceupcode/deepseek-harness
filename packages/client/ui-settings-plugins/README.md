...
- **A card still needs a browser bundle** — the browser half must be a `dsh.client` package built in the client module system's lazy-CJS factory format. The `clientBundle` preset that emits it lives in `../../../packages/client/tsdown.client.ts` rather than a published package, so a plugin outside this repository reproduces that build itself, against the `dsh-client-*` closure the dsh release publishes — the pack step's export check keeps that closure's payloads complete ([closure guarantee](../../../.agents/notes/implemented/process/2026-09-13-pack-time-export-completeness.md)).
