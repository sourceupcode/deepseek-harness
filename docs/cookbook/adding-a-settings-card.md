...
The shared preset is not a published package, so a browser half outside this repository reproduces the factory format itself; it builds against the published closure instead — the whole `dsh-client-*` set ships with every `dsh-v*` release, and the pack step refuses a member whose export map names an artifact its payload lacks ([the closure guarantee](../../.agents/notes/implemented/process/2026-09-13-pack-time-export-completeness.md)). The bundle-purity gate also rejects value imports across plugins, so a card cannot import this section's card chrome or its staged-form model — it renders its own, and owns its own staging and revision fencing. Both limits are recorded under [the section's known limitations](../../packages/client/ui-settings-plugins/README.md#known-limitations-and-deferred-work).
